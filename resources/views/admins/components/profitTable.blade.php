<div>
    @include('admins.components.profitForm')
    <div class="m-4">
        <div class="d-flex justidy-content-between mb-1">
            <h2 class="text-lg text-white font-medium mr-auto">
                <b>{{__('TABLE ARTIST PROFIT')}}</b>
            </h2>

            <div class="">
                <button class="btn btn-success" data-toggle="modal"
                    data-target="#profitAddModal">{{__('Add New Profit')}}</button>
            </div>
        </div>
        <div class="row mb-3">
            ...
            <table class="table table-striped table-hover table-sm table-dark">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @endforeach
                        <td class="align-middle">
                            <button type="button" data-toggle="modal" data-target="#profitUpdateModal"
                                wire:click="editprofit({{ $item->id }})" class="btn btn-primary m-1">
                                <i class="far fa-edit"></i>
                            </button>
                            <button type="button" data-toggle="modal" data-target="#deleteProfitModal"
                                wire:click="deleteProfit({{ $item->id }})" class="btn btn-danger m-1">
